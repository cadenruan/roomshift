...
Room width and depth plus the architecture are fixed for your proposal. Door configuration: ${JSON.stringify(layout.door)}. Its conservative clearance zone is ${JSON.stringify(doorZone(layout))}; do not place solid furniture in it. Window configurations: ${JSON.stringify(layout.windows)}. Keep tall furniture (above each window sill) out of that window's clearance zone. Wall decorations are fixed during rearrangement: ${JSON.stringify(layout.decorations)}.
